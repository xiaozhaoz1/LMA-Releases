...
/** 铁砧修理 — 搜索委托保留，GUI委托给 {@link WorldOutput#openGui}. */
@RuleAction
public final class AnvilRepairAction implements IAction {
    private static final List<TypedParam<?>> PARAMS = List.of(
        new TypedParam.IntParam("range", "搜索范围", 16)
    );

    @Override public String id() { return "anvil_repair"; }
    @Override public String displayName() { return "铁砧修理"; }
    ...
    public void execute(RuleContext ctx, Map<String, String> raw) {
        ...
        BlockPos pos = findAnvil(ctx.maid(), p.getInt("range"));
        if (pos == null) return;
