...
public final class StartTaskTool implements ITool<StartTaskTool.Params> {
    ...
    public String summary(EntityMaid maid) {
        return "Start a task for the maid. Call this DIRECTLY when the owner wants the maid to " +
               ...
               "target_count: omit or -1 = keep making until materials run out. " +
               "Specify a number ONLY when owner says exact quantity (e.g. 'make 16 sticks'). " +
               "Do NOT query inventory or recipes — the system handles everything internally.";
    }
    ...
    public LLMCallback onCall(String toolCallId, Params p, LLMCallback cb) {
        ...
        // v64: 清除 Brain dedup 缓存 (setTask 已在 TaskStateManager.init 中完成)
        data.remove(TaskKeys.FLOW_CACHED);
        LittleMaidMoreAction.LOGGER.info("[V18] [StartTaskTool] task '{}' submitted, target={}", p.taskType, p.target);
