...
/** 当前连段ID — getComboSeq() 返回 ResourceLocation。 */
@RuleCondition
public final class SlashbladeComboCondition implements ICondition {
    @Override public String key() { return "slashblade_combo"; }
    @Override public String displayName() { return "拔刀剑连段"; }
    ...
    @Override public String evaluate(RuleContext c, Map<String,String> r) {
        ...
        return c.maid().getMainHandItem().getCapability(ItemSlashBlade.BLADESTATE)
            .map(s -> s.getComboSeq().toString()).orElse("");
    }
}
