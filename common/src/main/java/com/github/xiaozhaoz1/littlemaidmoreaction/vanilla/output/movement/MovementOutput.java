...
public final class MovementOutput {
    ...
    public static void freezeAi(LivingEntity target, int ticks) {
        ...
        target.getPersistentData().putInt(TaskKeys.FREEZE_TICKS, ticks);
    }
    ...
    /**
     * v79.6: 模式化传送 (TeleportAction 组合吸收) — in_front/dodge_side/to_target/offset 分发。
     * target 用于 to_target (传送到目标实体位置 + 偏移); offset 相对 who 自身。
     */
    public static void teleportByMode(LivingEntity who, LivingEntity target, String mode,
                                      double distance, double ox, double oy, double oz) {
        switch (mode) {
            case "in_front" -> teleportWithMode(who, who, "in_front", distance, 0);
            case "dodge_side" -> teleportWithMode(who, who, "side", distance, ox != 0 ? ox : 0.5);
            case "to_target" -> {
                if (target == null) return;
                teleport(who, target.blockPosition().offset((int) ox, (int) oy, (int) oz));
            }
            case "offset" -> teleport(who, who.blockPosition().offset((int) ox, (int) oy, (int) oz));
            default -> { /* 未知模式 no-op */ }
        }
    }
