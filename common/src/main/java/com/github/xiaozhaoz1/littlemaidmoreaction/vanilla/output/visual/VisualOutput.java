package com.github.xiaozhaoz1.littlemaidmoreaction.vanilla.output.visual;
...
public final class VisualOutput {
    ...
    // === 动画 ===
    /**
     * v79.6: FULL 分段动画 (PlayAnimAction 组合吸收) — start/casting/end 非空则入 setup。
     */
    public static void playAnimFull(EntityMaid maid, String name,
                                    String start, String casting, String end) {
        java.util.Map<String, String> setup = new java.util.LinkedHashMap<>();
        if (!start.isEmpty()) setup.put("start", start);
        if (!casting.isEmpty()) setup.put("casting", casting);
        if (!end.isEmpty()) setup.put("end", end);
        playAnim(maid, name, "FULL", setup);
    }
...
//?} else {
//? if 1.20.1 {
        SoundEvent sound = BuiltInRegistries.SOUND_EVENT.getValue(rl);
//?} else {
...
//?} else {
        var sounds = BuiltInRegistries.SOUND_EVENT.entrySet().stream()
//?}
    ...
    // ── Phase 3: TLM 女仆粒子委托 (v38) ──

    /** 女仆专属粒子 — 委托 EntityMaid 内置粒子方法. 仅服务端调用, 客户端自动同步. */
    public static void spawnMaidParticle(EntityMaid maid, String type, int count) {
        switch (type) {
            case "heart" -> maid.spawnHeartParticle();
            case "explosion" -> maid.spawnExplosionParticle();
            case "bubble" -> maid.spawnBubbleParticle();
            case "health" -> maid.spawnRestoreHealthParticle(count);
            default -> {}
        }
    }
}
