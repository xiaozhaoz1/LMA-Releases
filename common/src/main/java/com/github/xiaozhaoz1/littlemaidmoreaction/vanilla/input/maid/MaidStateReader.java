...
//?} else {
        var s = m.getMainHandItem(); return s.isEmpty() ? "air" : BuiltInRegistries.ITEM.getKey(s.getItem()).toString();
//?}
    }
...
//?} else {
        var s = m.getOffhandItem(); return s.isEmpty() ? "air" : BuiltInRegistries.ITEM.getKey(s.getItem()).toString();
//?}
    }
...
//?} else {
        var o = m.getOwner(); return o != null ? BuiltInRegistries.ITEM.getKey(o.getMainHandItem().getItem()).toString() : "none";
//?}
    }
...
//?} else {
        var o = m.getOwner(); return o != null ? BuiltInRegistries.ITEM.getKey(o.getOffhandItem().getItem()).toString() : "none";
//?}
    }
...
//?} else {
            var key = eff.getEffect().getRegisteredName();
//?}
...
//?} else {
        for (var holder : s.getEnchantments().keySet()) {
            var loc = holder.getRegisteredName();
            if (loc.equals("minecraft:binding_curse") || loc.equals("minecraft:vanishing_curse")) return true;
        }
        return false;
//?}
    }
    ...
    /** 近战攻击范围平方值 */
    public static double getMeleeAttackRangeSqr(EntityMaid m, LivingEntity target) {
//? if 1.20.1 {
        return m.getMeleeAttackRangeSqr(target);
//?} else {
        // TLM 1.21 删除了 getMeleeAttackRangeSqr — 镜像其 isWithinMeleeAttackRange 公式
        int attackPlusDistance = m.getFavorabilityManager().getAttackDistancePlusByPoint(m.getFavorability());
        double attackDistance = m.getAttributeValue(net.minecraft.world.entity.ai.attributes.Attributes.ENTITY_INTERACTION_RANGE) + attackPlusDistance;
        return attackDistance * attackDistance;
//?}
    }
    /** 死亡掉落经验值 */
    public static int getExperienceReward(EntityMaid m) {
//? if 1.20.1 {
        return m.getExperienceReward();
//?} else {
        return m.getExperienceReward(m.level() instanceof net.minecraft.server.level.ServerLevel sl ? sl : null, m.getLastHurtByMob());
//?}
    }
    /** 音调高度 */
    ...
    /** 是否可被拴绳拴住 */
    public static boolean canBeLeashed(EntityMaid m, net.minecraft.world.entity.player.Player player) {
//? if 1.20.1 {
        return m.canBeLeashed(player);
//?} else {
        return m.canBeLeashed();
//?}
    }
    /** 女仆是否能拾取指定实体 */
...
//?} else {
    public static double getUseItemSpeed(EntityMaid m) { return m.getAttributeValue(com.github.tartaricacid.touhoulittlemaid.init.InitAttribute.MAID_USE_ITEM_SPEED); }
//?}
//? if 1.20.1 {
...
//?} else {
    public static double getCrossbowAttackSpeed(EntityMaid m) { return m.getAttributeValue(com.github.tartaricacid.touhoulittlemaid.init.InitAttribute.MAID_CROSSBOW_ATTACK_SPEED); }
//?}
//? if 1.20.1 {
...
//?} else {
    public static double getGunAttackSpeed(EntityMaid m) { return m.getAttributeValue(com.github.tartaricacid.touhoulittlemaid.init.InitAttribute.MAID_GUN_ATTACK_SPEED); }
//?}
//? if 1.20.1 {
...
//?} else {
    public static double getShootCooldown(EntityMaid m) { return m.getAttributeValue(com.github.tartaricacid.touhoulittlemaid.init.InitAttribute.MAID_SHOOT_COOLDOWN); }
//?}
//? if 1.20.1 {
...
//?} else {
    public static double getTridentCooldown(EntityMaid m) { return m.getAttributeValue(com.github.tartaricacid.touhoulittlemaid.init.InitAttribute.MAID_TRIDENT_COOLDOWN); }
//?}
//? if 1.20.1 {
...
//?} else {
    public static double getPickupRange(EntityMaid m) { return m.getAttributeValue(com.github.tartaricacid.touhoulittlemaid.init.InitAttribute.MAID_PICKUP_RANGE); }
//?}
//? if 1.20.1 {
