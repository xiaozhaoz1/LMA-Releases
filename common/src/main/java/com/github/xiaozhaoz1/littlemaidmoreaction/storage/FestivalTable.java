 ...
 *
 * <p>v79.63 架构审计: 自 {@code task/sense} **下沉 storage** — 它与 {@link FarmRegionStorage}
 * 同族 (静态数据表 + 被 task 读), 留在 task 层会让 {@code FestivalLoader} (storage) 反向 import task
 * (层级倒挂)。下沉后 loader 同包, 消费方 (task/sense · task/passive · commands) 走正向 task→storage。
 *
