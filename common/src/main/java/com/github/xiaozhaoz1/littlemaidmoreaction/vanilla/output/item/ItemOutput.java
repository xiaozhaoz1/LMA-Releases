package com.github.xiaozhaoz1.littlemaidmoreaction.vanilla.output.item;
...
//?} else {
            var key = entry.getKey().unwrapKey().map(k -> k.location()).orElse(null);
            if (key != null) map.put(key, entry.getIntValue());
//?}
        }
