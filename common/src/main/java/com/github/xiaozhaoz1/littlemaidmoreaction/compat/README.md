...
**作用**: 按第三方 mod 分包的兼容代码——Create 任务系 (crank/press/mix/power/running_belt/assembly)、AI 工具 (Numen 门控)、YSM 动画、CBC 火炮装填 (双平台, 2026-08-16 移植 1.21.1)、平台专属 (Numen neoforge)。
...
**代表**: create/task/* (Press/Mix/Crank/Power/RunningBelt + assembly) / ai/tool/* (GatedMaidTool 14 世界工具) / ysm/* (动画注入/输出)
...
- `ai/tool/` — GatedMaidTool 14 世界工具 (AI 操控门控); `ai/` — AiToolRegistration
...
- 兼容模块: `createbigcannons/` 在 common (CannonLoadPipeline/CannonLoadService, 双平台); 平台节点目录: `forge\src\main\java\...\compat\create\` — Create 平台接线 (forge 专属); `neoforge\src\main\java\...\compat\numen\` — NumenMaidBridge (neoforge 仅)
