...
- `WildKitsuneMilkConfig` — 9 项配置 (`kitsune_milk.toml`)
- `KitsuneMilkInteract` — 右键挤奶转换（驯服/主人三态 + 加好感）
- `KitsuneMilkBaubleRegistry` — `bindMaidBauble` 注册两个 bauble

**依赖方向**: TLM `api.bauble` + LMA `bauble/`(基座) + `config`。
...
| 已驯服 + 别人 | 不能挤奶 |
| 未驯服 | 野生奶(副开关开)/奶桶(副开关关) + 爱心粒子 (不打人不哈气不加好感 — 用户裁定) |

挤奶本身无 CD; 加好感 5min CD (仅好感)。
