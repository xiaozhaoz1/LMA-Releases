 ...
 *
 * <p>玩家在 TLM 任务栏选择 "AI 操控" → 首次执行写 {@link AiControlGate} 标记
 * (女仆 AI 对话获得世界操作权限: 移动/挖掘/战斗/切任务等工具暴露);
 * 取消任务 → onCleanup 删标记 (闭环, 权限收回)。
 *
 * <p>无实质动作 — 开关语义由任务生命周期表达 (executor 恒 CONTINUE; v74.1 修正:
 * isLongRunning 只启用超时看门狗检查, 非豁免 — needsGameTick 心跳才是续命;
 * 无目标任务永不走 Brain 执行路径, 必须由 TaskTickHandler needsGameTick 分支心跳)。
 ...
 */
public final class AiControlPipeline implements TaskPipeline {
    ...
    /** v74.1: 每 tick 心跳续命 — 超时看门狗 (1200t) 依赖 lastTick 刷新, tick() 默认空实现无副作用 */
    @Override public boolean needsGameTick() { return true; }

    @Override
    public PipelineResult validate(ServerLevel level, EntityMaid maid, PipelineContext ctx) {
        // v75.3: 注册级门控 (TaskRegistry — 未装 Numen 不注册), validate 冗余防御
    ...
    /** v75.1: 设置 GUI "变成假人" 动作 (任务须运行中 — gate on 由桥侧校验) */
    @Override
    public boolean handleConfigAction(EntityMaid maid, byte action, CompoundTag payload) {
        if (action == ACTION_TRANSFORM) {
            if (TRANSFORM_ACTIVATOR != null) {
                ...
                // v77.7: 前置提示 — Numen (假人) / YSM (OpenYSM 或混淆版均可 — 混淆版走命令通道)
                String missing;
                if (!com.github.xiaozhaoz1.littlemaidmoreaction.compat.NumenCompat.isInstalled()) {
                    missing = "Numen";
        ...
        // 引擎通用动作 (复制默认实现 — 不依赖 interface super 解析)
    ...
    /** 任务设置 GUI (v74) — LLM 模型/声线名称 (TLM 任务设置标签页入口) */
    @Override
