...
public final class MixService {
    ...
    public static boolean hasRecipe(Level level, BlockPos pos) {
        ...
        return hasMatchingRecipe(level, basin);
    }

    /** 1.20: getAllRecipesFor 返回 Recipe; 1.21: RecipeHolder (值经 .value()) */
    private static boolean hasMatchingRecipe(Level level, BasinBlockEntity basin) {
//? if 1.20.1 {
        for (Recipe<?> r : level.getRecipeManager().getAllRecipesFor(AllRecipeTypes.MIXING.getType())) {
