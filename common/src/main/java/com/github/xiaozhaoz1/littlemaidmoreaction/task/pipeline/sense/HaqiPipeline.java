 ...
 */
public final class HaqiPipeline implements TaskPipeline {

    /** 音频清单 (注册于 LmaSounds, v79.9) */
    ...
    private static final Map<String, Integer> SOUND_TICKS = Map.of(
    ...
    /** v79.17: 挥击倒计时 (进入 LOOK 后延迟, 0=不挥 / -1=已挥) */
    private static final String KEY_HIT_TICKS = "hit_ticks";
    /** v79.20: 目标类型 (maid=对女仆 / owner=对主人; 向后兼容: 读空默认 maid) */
    ...
    /** v79.18: 哈气动画名 — 对应 haqi.animation.json 内动画 key "haqi" (YSM 模型包内需同名动画) */
    public static final String HAQI_ANIM = "haqi";
    /** v79.20.4: 对主人哈气动画 — maimeng.animation.json 内 key "maimeng" (用户裁定: 对女仆仍 haqi 不动) */
    public static final String HAQI_OWNER_ANIM = "maimeng";

    /**
     * v79.12: 触发双通道 ({@link HaqiTrigger} 直接扫描 / onSignal) 与状态机共享的状态数据。
    ...
    /** v79.16: 哈气结束清理 (pipelineData) — 蹲下不做 (YSM 模型不响应 pose CROUCHING, 用户裁定删除) */
    @Override
    public void onCleanup(EntityMaid maid) {
        // v79.18: YSM 动画停止 (cancelPassive 先调 onCleanup, 覆盖全部退出路径)
        YsmOutput.stopRoulette(maid);
        // v79.18: AnimExecute fallback freezeAI=true 置 IS_PANICKING — 结束必须解除 (Brain memory 闭环)
        BrainHelper.unfreeze(maid);
        // v79.18: 动画键闭环 — 清 ANIM_MODE 并同步 → 客户端 provider 返回 null →
        // AnimationManager: CASTING→NONE 允许终止 → clearAnimationCache (FULL 循环强制停止, 防收回女仆后继续播)
        var root = maid.getPersistentData();
        root.remove(TaskKeys.ANIM_MODE);
    ...
    public PipelineResult validate(ServerLevel level, EntityMaid maid, PipelineContext ctx) {
        ...
        return PipelineResult.ok("", Set.of(Signals.envOf(EnvSignal.MAID_NEARBY)));
    }
    ...
    public void onSignal(EntityMaid maid, EnvSnapshot snap, String signalId) {
        // v79.20: 总开关 + 对主人二级开关, 任一开则进入 (内部再分别门控)
        ...
        if (target != null && RANDOM.nextDouble() < PassiveTaskConfig.HAQI_CHANCE.get()) {
            // 锁定目标 + 提交 (v79.12: 写 pipelineData compound — 原写根导致状态机读空立即取消)
    ...
    public void tick(ServerLevel world, EntityMaid maid) {
        ...
        // v79.20: 目标类型 (向后兼容: 旧存档无 target_type → 按 maid 处理)
        ...
        switch (state) {
            ...
            case LOOK -> {
                // v79.16b: LOOK_TARGET 记忆 — TLM LookAtTargetSink 每实体 tick 读它转头 (原 lookAt 只设 yRot 被覆盖)
                ...
                // v79.16b: yaw 三轴直设 (身体+头转向目标; lookAt 不设 yHeadRot)
                ...
                // v79.16: 防随机走 (pin WALK_TARGET 脚格 + 停导航)
                NavigationUtil.keepAlive(world, maid);
                maid.getNavigation().stop();
                // v79.17: 挥击倒计时 → 到 0 执行一次真实攻击 (swing 动画 + mobAttack 伤害 + 挥击音)
                int hitTicks = data.getInt(KEY_HIT_TICKS);
                if (hitTicks > 0) {
                    hitTicks--;
                    if (hitTicks == 0) {
                        doHit(maid, target, targetType);
                        hitTicks = -1; // 已挥, 防重复
                    }
    ...
    /**
     * v79.20: 进入 LOOK — 按目标类型分流:
     * <ul>
     *   <li>maid (原版): 随机 SOUND_NAMES + playSound, 总时长 = 基础 + 音频时长 (逐行不变)</li>
    ...
    private static void enterLook(ServerLevel world, EntityMaid maid, CompoundTag data, String targetType) {
        ...
        if (TARGET_OWNER.equals(targetType)) {
            ...
            // v79.20: 概率挥击 — 掷骰命中则延迟 15t 后拍一下 (伤害走对主人配置, 主人不反击)
            ...
            // 对女仆: 原版分支逐行不变
            String sound = SOUND_NAMES[RANDOM.nextInt(SOUND_NAMES.length)];
            playSound(maid, sound);
            int base = PassiveTaskConfig.HAQI_DURATION_TICKS.get();
            int audio = SOUND_TICKS.getOrDefault(sound, 60);
            data.putInt(KEY_AUDIO_TICKS, base + audio);
            // v79.17: 概率挥击 — 掷骰命中则延迟 15t 后拍一下 (伤害走配置, 默认 1 点血)
            data.putInt(KEY_HIT_TICKS,
                    RANDOM.nextDouble() < PassiveTaskConfig.HAQI_HIT_CHANCE.get()
                            ? HIT_DELAY_TICKS : 0);
            total = base + audio;
        }
        // v79.20: 表情气泡 — 对女仆/对主人子集随机, 与语音随机相互独立
        MaidEmojiApi.send(maid, TARGET_OWNER.equals(targetType) ? MaidEmojiType.OWNER : MaidEmojiType.MAID);
        // v79.18 诊断: 触发女仆模型状态 (isYsm 服务端判断 — YSM 通道分流依据)
        ...
        // v79.18: 哈气动画 — AnimExecute FULL 双通道分流 (YSM=playRouletteAnim /
        // TLM=ISS geckolib: START 播一次 → CASTING 循环 (durCasting=LOOK 总时长×3 —
        // 用户裁定: 动画持续时长 = 哈气总时长 3 倍, 哈气期间持续趴着) → END 播一次 → 自动停;
        // FULL 而非 INSTANT — INSTANT 的 LOOP 无法停止 (AnimationManager INSTANT→NONE 播完为止, LOOP 永播)
        // v79.20.4: 对主人播 maimeng 动画, 对女仆仍 haqi (用户裁定)
        // v79.25.1: 删 v79.20.6 isYsmModel 分流 — 当时误判 "TLM 模型小写骨" 造了 haqi_vanilla/maimeng_vanilla,
        // 实际 TLM 模型 (winefox.json) 骨名 PascalCase (Head/LeftArm/...), vanilla 小写动画 0 匹配 →
        // geckolib 播了但骨不动 ("maimeng 播不出" 真因; haqi 能播是巧合 — AllBody 是 TLM 通用根骨)。
        // maimeng/haqi 骨名 PascalCase, TLM/YSM 双模型均匹配 → 统一播同一个 (错题 #134)
        String anim = TARGET_OWNER.equals(targetType) ? HAQI_OWNER_ANIM : HAQI_ANIM;
    ...
    /**
     * v79.17: 挥击 — 真实攻击链 (挥击动画 + mobAttack 伤害 + 原版挥击音) 但伤害固定配置值 (默认 1 点血, 不致命)。
     * TLM 实证: 女仆打女仆不会引发反击 (DefaultMonsterType FRIENDLY 排除 TamableAnimal, canAttack 恒 false)。
     * v79.20: 目标类型分流伤害 (owner → HAQI_HIT_DAMAGE_TO_OWNER); 打主人不反击 (玩家无自动反击)。
     */
    private static void doHit(EntityMaid maid, LivingEntity target, String targetType) {
        maid.swing(InteractionHand.MAIN_HAND);
        // v79.20.4: 对主人用魔法伤害 (绕护甲) — mobAttack 1.0 伤害被满甲玩家减到 0.2, 肉眼无感 (用户实测"没效果");
        // 魔法伤害 LivingEntity.hurt 不走护甲减伤 (isMagic → 跳过 getDamageAfterArmorAbsorb), 掉血可见。
        // 对女仆保持 mobAttack 真实攻击链不动 (v79.17)。
        if (TARGET_OWNER.equals(targetType)) {
    ...
    private static void playSound(EntityMaid maid, String name) {
