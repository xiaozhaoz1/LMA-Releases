...
final class MaidChainState {
    ...
    long lastNearScan;
    /** v79.64 中扫 (10 格球) 节流 — 0=未扫过 (用户裁定: 女仆走不快 ⇒ 100t 一次 ✓) */
    long lastMidScan;
    /** v79.64 远扫 (16 格球) 节流 — 0=未扫过 (用户裁定: 250t 一次 ✓) */
    long lastFarScan;
    /**
     * v79.64 **当前目标** (pos.asLong) — 节流只作用于"重新搜索"，**导航要靠它每 tick 续期** ✓
     * <p>背景：三层节流后 `navigate` 不再被周期调用 ⇒ `WALK_TARGET` 无人续 ⇒ 女仆"一步不走" ✗
     * （旧实现每 20t 重扫一次 ⇒ 等于每 20t 重发导航 ✓ 所以旧版能走 ✓）。{@link Long#MIN_VALUE} = 无目标。
     */
    long currentTarget = Long.MIN_VALUE;
    ...
    /**
     * 清掉靠近某点的跳过条目 (v79.63 用户裁定: **玩家破坏方块 → 失效重算**) — 返回清除数。
     *
     * <p>背景: 女仆跟着玩家下矿, 地形变化比 SKIP_TTL(10 秒)/TTL_LONG(30 秒) 快 ⇒ 靠"失效"而非"到期"
     * 更准; 玩家挖开那堵墙后, 原本"不可达"的矿应立刻可再选。
     */
    int clearSkipsNear(net.minecraft.core.BlockPos pos, long r2) {
        int n = 0;
        java.util.Iterator<Long> it = skipped.iterator();
        while (it.hasNext()) {
            long l = it.next();
            if (net.minecraft.core.BlockPos.of(l).distSqr(pos) <= r2) {
                it.remove();
                skipAt.remove(l);
                failCounts.remove(l);   // 失败计数同清 — 地形变了应重新计
                n++;
            }
        }
        return n;
