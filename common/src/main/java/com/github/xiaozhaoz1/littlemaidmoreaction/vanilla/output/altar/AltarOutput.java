package com.github.xiaozhaoz1.littlemaidmoreaction.vanilla.output.altar;
...
public final class AltarOutput {
    ...
    public static boolean tryTriggerCraft(EntityMaid maid, TileEntityAltar ref) {
        ...
        var recipeOpt = maid.level().getRecipeManager().getRecipeFor(
                InitRecipes.ALTAR_CRAFTING.get(), craftingInput, maid.level());
        if (recipeOpt.isEmpty()) return false;

        var recipe21 = recipeOpt.get().value();
        BlockPos center21 = getCenterPos(slotPositions, ref.getBlockPos());
        if (maid.level() instanceof ServerLevel sl)
            recipe21.spawnOutputEntity(sl, center21.above(2), items);
        //?}
