 ...
 *
 * <p>v79.61x 死信号同族清理 + v79.62.5 温度边沿删除 (TEMP_COLD/HOT/NORMAL — TLM 覆盖, 用户裁定) 后,
 * 只测剩 5 个有消费方检出 (SNOWING/WEATHER_CLEAR/DARKNESS/MAID_NEARBY + FESTIVAL/RARE_BIOME 由广播器覆盖)。
 */
class EnvEdgeDetectorTest {

    private static final EnvEdgeDetector.EnvConfig CFG = new EnvEdgeDetector.EnvConfig(7);

    /** WorldInfo helper — 默认全 false/0/空串, 按需覆写 */
    private static EnvSnapshot.WorldInfo info(boolean day, boolean raining, String precip,
                                              int light, String dimension,
                                              String timeSeg, String biomeId, String structsAt) {
        return new EnvSnapshot.WorldInfo(day, raining, false, 0, light, dimension,
                precip, 1000, timeSeg, biomeId, structsAt);
    }

    private static EnvSnapshot.WorldInfo base() {
        return info(true, false, "NONE", 12, "minecraft:overworld", "DAY", "minecraft:plains", "");
    }
    ...
    void snowing_onset() {
        var snowing = info(true, true, "SNOW", 12, "minecraft:overworld", "DAY", "minecraft:plains", "");
    ...
    void weather_clear() {
        var raining = info(true, true, "RAIN", 12, "minecraft:overworld", "DAY", "minecraft:plains", "");
    ...
    void darkness_edge() {
        var dark = info(true, false, "NONE", 3, "minecraft:overworld", "DAY", "minecraft:plains", "");
